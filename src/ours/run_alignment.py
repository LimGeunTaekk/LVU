...
def parse_arguments():
    ...
    parser.add_argument('--response_file', type=str, default='./llms/mlvu.json',help='path of output scores and frames')
...
def main(args):
    ...
    with open(args.response_file,'r') as f:
        reponse_list = json.load(f)

    for data in tqdm.tqdm(datas):
        
        identify_key = 'question_id'
        if identify_key not in data.keys():
            if 'id' in data.keys():
                identify_key = 'id'
        ...
        try:
            ...
            text = data['question']  
            response = reponse_list[data[identify_key]]
            ...
            if args.extract_feature_model == 'blip':
                ...
                for j in range(frame_nums):
                    ...
                    with torch.no_grad():
                        blip_output_q = model({"image": img, "text_input": txt}, match_head="itm")         
                        blip_output_cxt = model({"image": img, "text_input": context}, match_head="itm")               
        
                    blip_scores_q = torch.nn.functional.softmax(blip_output_q, dim=1)
                    blip_scores_cxt = torch.nn.functional.softmax(blip_output_cxt, dim=1)

                    question_score = blip_scores_q[:, 1].item()
                    context_score = blip_scores_cxt[:,1].detach().cpu().numpy().tolist()
